  ...
  return (
    <div className="space-y-6">
      <section className="surface rounded-[28px] p-5">
        <div className="text-xs uppercase tracking-[0.18em] text-[var(--color-blue)]">Create payment</div>
        <h3 className="mt-2 font-[family-name:var(--font-heading)] text-2xl">One-time disbursement</h3>
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-[1fr_0.7fr_0.7fr_auto]">
          ...
          <button type="button" onClick={() => createPayment().catch(() => undefined)} className="rounded-full bg-[var(--color-sidebar)] px-5 py-3 text-sm font-semibold text-white">Create</button>
        </div>
      </section>
      ...
      <section className="surface overflow-hidden rounded-[30px]">
        <div className="flex items-center justify-between border-b border-[var(--color-line)] px-5 py-4 sm:px-6">
          <h3 className="font-[family-name:var(--font-heading)] text-2xl">Live payments</h3>
          <button type="button" onClick={() => runAction("/api/payments/reconcile").catch(() => undefined)} className="rounded-full border border-[var(--color-line)] px-4 py-2 text-sm font-semibold">Reconcile</button>
        </div>
            ...
            <tbody>
              {payments.map((payment) => (
                <tr key={payment.id}>
                  ...
                  <td className="border-t border-[var(--color-line)] px-5 py-4">{payment.recipient?.fullName ?? "No recipient"}</td>
                  <td className="border-t border-[var(--color-line)] px-5 py-4">{payment.amount}</td>
                  ...
                  <td className="border-t border-[var(--color-line)] px-5 py-4">
                    {payment.status === "FAILED" ? (
                      <button type="button" onClick={() => runAction(`/api/payments/${payment.id}/retry`).catch(() => undefined)} className="text-sm font-semibold text-[var(--color-blue)]">Retry</button>
                    ) : payment.status === "SCHEDULED" ? (
                      <button type="button" onClick={() => runAction(`/api/payments/${payment.id}/execute`).catch(() => undefined)} className="text-sm font-semibold text-[var(--color-blue)]">Execute</button>
                    ) : null}
                  </td>
                </tr>
              ))}
